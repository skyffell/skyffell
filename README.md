ʜɪ ᴛʜᴇʀᴇ! 👋 ɪ'ᴍ ᴀʟᴇxᴇʏ

Я Frontend-разработчик с опытом работы с HTML, CSS и JavaScript. Добро пожаловать в мой профиль на GitHub! 🚀

## 👨‍💻 ᴀʙᴏᴜᴛ ᴍᴇ
🌱 Сейчас я работаю над созданием красивых и функциональных веб-интерфейсов  
💡 Мне нравится превращать сложные задачи в элегантные решения  
🎨 Passionate about creating intuitive user experiences  
🏠 Создаю цифровые пространства для комфортной жизни  

## ⚙️ ᴛᴇᴄʜ ꜱᴛᴀᴄᴋ
**Языки:** HTML, CSS, JavaScript  
**Инструменты:** Git, VS Code, Chrome DevTools  
**Методологии:** Responsive Web Design, Cross-browser compatibility  
**Изучаю:** TypeScript, Node.js  

## 💻 ᴘʀᴏᴊᴇᴄᴛꜱ

### 1. [Flock House](https://flockhouse.ru/)
**Описание:** Современная платформа для поиска и аренды жилья  
**Технологии:** HTML, CSS, JavaScript, Adaptive Design  
**Особенности:** Удобный интерфейс поиска, фильтрация, карточки объектов  
**Статус:** Активный проект с реальными пользователями

### 2. [Парсер расписания МГКЦТ](https://github.com/skyffell/mgkct-schedule-parser)
**Описание:** Веб-приложение для автоматического парсинга расписания занятий колледжа  
**Технологии:** Flask, BeautifulSoup, HTML, CSS, JavaScript  
**Особенности:** Адаптивный дизайн, темы (темная/светлая), мобильная версия  
**Демо:** [schedulemskdt.ru](https://schedulemskdt.ru/)

### 3. [N-code Portfolio](https://github.com/skyffell/n-code-portfolio)  
**Описание:** Персональное портфолио с современным дизайном  
**Технологии:** HTML5, CSS3, JavaScript  
**Особенности:** Интерактивные элементы, плавные анимации, адаптивная верстка

## 🔥 ᴍʏ ɢɪᴛʜᴜʙ ꜱᴛᴀᴛꜱ

![GitHub Streak](https://streak-stats.demolab.com/?user=skyffell&theme=radical)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=skyffell&layout=compact&theme=radical)

## ✉️ ʟᴇᴛ'ꜱ ᴄᴏɴɴᴇᴄᴛ!
[![Telegram](https://img.shields.io/badge/Telegram-2CA5E0?style=for-the-badge&logo=telegram&logoColor=white)](https://t.me/skyffell)
[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:alesha.300053@gmail.com)
[![Website](https://img.shields.io/badge/Website-Portfolio-FF6B6B?style=for-the-badge&logo=house&logoColor=white)]([https:///](https://skyffell.github.io/n-code-portfolio/))

*"Создаю цифровые миры, где код встречается с креативностью"* 🏠✨

